...
**第 2 步：搜索新闻 → 元数据收集（不进知识库）**

搜新闻时，系统把新闻存为元数据，而不是知识库文档：
...
**关键区别：新闻不进 KB，不向量化，只在查询时作为补充上下文显示。**
...
在「数据源」页面，点目录旁的“分析并导入”按钮。系统会：
1. 读取目录下的文件（.jsonl / .txt / .json）
2. 对每篇文档运行 IngestionAgent + ExtractionAgent
