...
| **智能查询** | 知识库问答 | 输入问题 | 系统从知识库匹配最相关的文档，LLM 生成带引用的回答。混合检索（BM25+向量+RRF+Rerank） |
| **智能查询** | 深度调研 | 输入话题，切换到“深度调研”模式 | 系统自动抓取实时新闻 + 结合知识库 + AI 深度分析 + 生成结构化报告 + 事实核查。走完整 5 阶段 Pipeline |
| **智能查询** | K线分析 | 输入股票代码或名称 | 生成 MACD/RSI/KDJ/布林带技术分析报告 |
| **深度分析** | 新闻解读 | 粘贴新闻文本 | Agent 链 (AnalysisAgent → ScoringAgent)：多维影响评估 + 关键信号 + 风险提示 + 防幻觉校验 |
| **深度分析** | 话题调研 | 输入话题关键词 | Agent 链：抓新闻 + KB 检索 + 子话题聚类 + 情绪趋势 + 反向信号 + 防幻觉校验 |
| **深度分析** | 追问 | 分析完成后输入追问 | 基于原始分析上下文的多轮对话，支持对新闻/话题结果深入追问 |

> **智能路由**：查询自动分类为 4 种意图 + general 兜底（kline / event_impact / report / news / general），系统选择最优 Agent 执行链，无需手动指定。深度分析页签的 `news` 和 `deep_topic` 意图直接走 Agent 链，不经过 Pipeline。
>
> **智能查询双模式**：智能查询页签提供两种模式——“知识库问答”（混合检索 + LLM 回答）和“深度调研”（5 阶段 Pipeline 全链路）。用户界面无技术术语，每个模式都有步骤提示（①②③）说明操作流程。
>
