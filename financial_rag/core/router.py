...
class CommandRouter:
    ...
    def _cmd_demo(self, args=None) -> None:
        print("=" * 60)
        print("Financial RAG — 演示 (阿里百炼全链路)")
        print("=" * 60)

        print(f"\n[配置]")
        print(f"  LLM:      {self._cfg.llm.model} {'(已配置)' if self._has_key else '(未配置)'}")
        print(f"  Embedding: {self._cfg.llm.embedding_model}")
        print(f"  Rerank:    {self._cfg.llm.rerank_model}")
    ...
    """
        ...
        if self._has_key:
            try:
                ...
                r.index(test_docs, precompute_embeddings=True)

                print("[全链路检索测试 (含打分)]")
                for q in ["茅台2024年盈利情况", "茅台各产品线营收"]:
                    results, card = r.search_with_scores(q, top_k=3, use_rerank=True)
                    print(f"\n  Q: {q}")
                    for item in results:
                        print(f"    [{item['retriever']}] relev={item.get('relevance_level', '?')} "
                              f"score={item.get('score', 0):.4f} | {item['text'][:60]}")
                    card.print_summary()
                print()
            except Exception as e:
        ...
        print("使用方法:")
        print("  python -m financial_rag.main pipeline <查询>     # 统一Pipeline")
