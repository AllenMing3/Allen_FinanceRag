...
<analysis_framework>
按以下步骤推进（每一步必须引用具体数值）：
Step 1（趋势判断）：观察 MA5/MA10/MA20 排列 → 多头排列（MA5>MA10>MA20）= 多头趋势；空头排列 = 空头趋势；交叉缠绕 = 震荡
Step 2（动量确认）：MACD 金叉/死叉 + RSI 超买(>70)/超卖(<30) → 确认趋势强度
Step 3（波动区间）：布林带位置（价格在上轨附近=超买，下轨附近=超卖）+ KDJ 交叉信号 → 判断短期拐点
Step 4（量价配合）：放量突破 vs 缩量回调 → 判断突破有效性
Step 5（综合结论）：支撑位/压力位 + 操作建议 + 风险提示
</analysis_framework>

<anti_hallucination>
1. 每个判断必须引用提供的具体数据（如"MA5 上穿 MA10""DIF 金叉 DEA"），不要写"技术指标显示积极信号"这种空话
2. 支撑位/压力位必须基于提供的价格数据计算，不要编造价位
3. 如果指标之间矛盾（如 MACD 看多但 RSI 超买），必须指出矛盾而非忽略
4. 操作建议必须基于分析结论，不要写"建议关注"这种空话
</anti_hallucination>

<edge_cases>
- 指标矛盾（如 MACD 金叉 + RSI 超买）：指出矛盾，给出"谨慎看多"等中间判断
- 数据不足（如缺少某个指标）：基于已有指标分析，注明缺失指标
- 震荡行情（均线缠绕）：明确标注"震荡格局"，给出区间
</edge_cases>

<negative_examples>
错误示范1："技术指标显示积极信号" → 不及格。必须说明是哪个指标的什么信号
错误示范2："短期可能上涨" → 不及格。必须说明依据（如"MACD 金叉 + 放量突破 MA20"）
错误示范3：支撑位写"1800 元附近"但数据中没有这个价位 → 不及格。必须基于实际数据
</negative_examples>

<rubric>
90分：每个判断引用具体数值 + 指标矛盾已识别 + 支撑/压力位有数据依据
60分：趋势判断正确但缺乏具体数值引用
不及格：全是"技术面积极""建议关注"等空话，无任何具体数据
</rubric>
    ...
    try:
        ...
        parsed = caller.call_json(
            ...
            system=(
                ...
                "天数映射规则：'最近/近期'→30天，'半年'→120天，'一年'→240天，'三个月'→60天。"
                "只提取查询中明确出现的实体作为 keyword，不要联想扩展。"
            ),
            max_tokens=60,
        )
        if parsed:
    ...
    try:
        ...
        resp = caller.call(
            ...
            system=(
                ...
                "不要编造数据，只做基于现有数据的判断。"
                "分析框架：均线排列判断趋势→MACD/RSI确认动量→布林带判断波动→综合结论。"
                "每个判断必须引用具体数值，禁止写'技术面积极'这类空话。"
            ),
            max_tokens=300,
        )
        return resp.content
